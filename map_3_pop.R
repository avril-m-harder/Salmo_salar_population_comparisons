...
al <- 1
line.w <- 0.5
green <- "gray95" ## other greens: springgreen4  #B5D383  mediumseagreen #ADF2CB #76C599
blue <- '#DFF7FF' ## other blues: cadetblue1 #98CAC2  lightblue2  #DFF7FF
...
ggplot(data = states) +
  ...
  geom_sf(data = SE, fill=alpha(blue, al), col='#992913', lwd=line.w) + ## other blues: cadetblue1 #98CAC2
...
##### map with greater extent for talks
pdf('/Users/Avril/Desktop/map.pdf', width=5, height=5)
ggplot(data = states) +
  theme_classic() +
  theme(panel.background = element_rect(fill=alpha(blue, al)),
        axis.text=element_text(size=10, colour='black'),
        ...
        panel.border=element_rect(fill=NA)) +
  geom_sf(fill=green, col='black', lwd=0.25) + ## other greens: springgreen4  #B5D383
  # geom_sf(data = us.can, col='black', lwd=0.75) +
  ## add study waterbodies
  geom_sf(data = LC, fill=alpha(blue, al), col='#1571a9', lwd=line.w) +
  ## add all rivers
  # geom_sf(data = rivs, fill=NA, col='cadetblue1') +
